...
save(test.int, file = "merged_dataset.RData")
```
...
identical(names(pred.hpca), data.sce)
data.sce$pred.hpca <- pred.hpca$labels
data.sce$pred.im <- pred.im$labels
data.sce$pred.nh <- pred.nh$labels

ct <- data.frame(cbind(Seurat = data.sce$seurat_clusters, 
                       HPCA = data.sce$pred.hpca, 
                       IM = data.sce$pred.im, 
                       NH = data.sce$pred.nh))
rownames(ct) <- colnames(data.sce)
